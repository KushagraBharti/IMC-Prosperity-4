...
class Trader:
    ...
    TREND_DELTA_CAP = -700.0
    TREND_DELTA_TRIGGER = 0.65
    TREND_DELTA_MAX_EDGE_SHIFT = 2.5
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            momentum = self.velvet_momentum(cache)
            if momentum > self.TREND_DELTA_TRIGGER and net_delta < self.TREND_DELTA_CAP and delta > 0.15:
                relief = min(self.TREND_DELTA_MAX_EDGE_SHIFT, (self.TREND_DELTA_CAP - net_delta) / 150.0)
            else:
                relief = 0.0
            original_position = position
            orders: List[Order] = []
            ...
            for ask_price, ask_volume in book["sell_orders"]:
                edge = fair_adj - ask_price
                if edge >= max(0.5, edge_threshold - relief):
