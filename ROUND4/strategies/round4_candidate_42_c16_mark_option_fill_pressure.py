...
class Trader:
    ...
    OPTION_FILL_PRESSURE_SCALE = 0.18
    OPTION_FILL_PRESSURE_MAX_EDGE = 3.0
    OPTION_PRESSURE_SELLERS = {"Mark 01", "Mark 14"}
    ...
    def update_option_fill_pressure(self, cache: Dict[str, object], state: TradingState) -> None:
        ...
        for symbol, trades in (getattr(state, "own_trades", {}) or {}).items():
            ...
            for trade in trades:
                if (
                    self.trade_attr(trade, "buyer", "") == "SUBMISSION"
                    and self.trade_attr(trade, "seller", "") in self.OPTION_PRESSURE_SELLERS
                ):
