...
class Trader:
    ...
    OPTION_FILL_PRESSURE_DECAY = 0.62
    OPTION_FILL_PRESSURE_SCALE = 0.12
    OPTION_FILL_PRESSURE_MAX_EDGE = 2.5
    OPTION_FILL_PRESSURE_PRODUCTS = {"VEV_5200", "VEV_5300", "VEV_5400", "VEV_5500"}
    ...
    def run(self, state: TradingState):
        ...
        self.update_option_fill_pressure(cache, state)
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            pressure = self.option_fill_pressure(cache, symbol) if symbol in self.OPTION_FILL_PRESSURE_PRODUCTS else 0.0
            original_position = position
            orders: List[Order] = []
            ...
            for ask_price, ask_volume in book["sell_orders"]:
                edge = fair_adj - ask_price
                if edge >= edge_threshold + pressure:
    ...
    def update_option_fill_pressure(self, cache: Dict[str, object], state: TradingState) -> None:
        pressure = cache.get("option_fill_pressure")
        if not isinstance(pressure, dict):
            pressure = {}
        for symbol in self.ACTIVE_VOUCHERS:
            pressure[symbol] = float(pressure.get(symbol, 0.0)) * self.OPTION_FILL_PRESSURE_DECAY
        for symbol, trades in (getattr(state, "own_trades", {}) or {}).items():
            if symbol not in self.STRIKES:
                continue
            for trade in trades:
                if self.trade_attr(trade, "buyer", "") == "SUBMISSION":
                    quantity = max(1.0, float(self.trade_attr(trade, "quantity", 1.0)))
                    pressure[symbol] = float(pressure.get(symbol, 0.0)) + math.sqrt(quantity)
        cache["option_fill_pressure"] = pressure

    def option_fill_pressure(self, cache: Dict[str, object], symbol: str) -> float:
        pressure = cache.get("option_fill_pressure")
        if not isinstance(pressure, dict):
            return 0.0
        raw = float(pressure.get(symbol, 0.0))
        return min(self.OPTION_FILL_PRESSURE_MAX_EDGE, self.OPTION_FILL_PRESSURE_SCALE * raw)
