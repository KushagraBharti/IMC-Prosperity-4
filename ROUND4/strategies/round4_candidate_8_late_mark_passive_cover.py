...
class Trader:
    ...
    OPTION_MARK_WEIGHTS = {
        "VEV_5200": {"Mark 22": -0.32},
        "VEV_5300": {"Mark 14": 0.32, "Mark 22": -0.18},
        "VEV_5400": {"Mark 14": -0.38, "Mark 01": 0.10},
    }
    MARK_PASSIVE_COVER_START = 86_000
    MARK_PASSIVE_COVER_TRIGGER = {
        "VEV_5200": 0.30,
        "VEV_5300": 0.08,
        "VEV_5400": 0.08,
    }
    MARK_PASSIVE_COVER_TARGET = {
        "VEV_5200": -255,
        "VEV_5300": -245,
        "VEV_5400": -245,
    }
    MARK_PASSIVE_COVER_SIZE = {
        "VEV_5200": 10,
        "VEV_5300": 10,
        "VEV_5400": 10,
    }
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            mark_orders, position = self.trade_late_mark_passive_cover(symbol, book, position, timestamp, option_mark_signals.get(symbol, 0.0))
    ...
    def trade_late_mark_passive_cover(
        self,
        symbol: str,
        book: Dict[str, object],
        position: int,
        ...
        orders: List[Order] = []
        if timestamp < self.MARK_PASSIVE_COVER_START or symbol not in self.MARK_PASSIVE_COVER_TARGET:
            return orders, position
        if mark_signal < self.MARK_PASSIVE_COVER_TRIGGER[symbol] or position >= self.MARK_PASSIVE_COVER_TARGET[symbol]:
            return orders, position
        ...
        capacity = self.MARK_PASSIVE_COVER_TARGET[symbol] - position
        qty = min(self.MARK_PASSIVE_COVER_SIZE[symbol], capacity)
        if qty > 0:
            orders.append(Order(symbol, int(bid_price), int(qty)))
        return orders, position
