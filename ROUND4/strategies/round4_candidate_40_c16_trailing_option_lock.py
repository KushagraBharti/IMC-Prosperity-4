...
class Trader:
    ...
    OPTION_TRAIL_LOCK_MIN_PEAK = 82_000.0
    OPTION_TRAIL_LOCK_DRAWDOWN = 1_250.0
    OPTION_TRAIL_LOCK_PRODUCTS = {
        "VELVETFRUIT_EXTRACT",
        "VEV_4000",
        "VEV_4500",
        "VEV_5000",
    ...
    def run(self, state: TradingState):
        ...
        if bool(cache.get("option_trail_lock", False)) or self.should_lock_option_trail(cache):
            cache["option_trail_lock"] = True
            for product in self.OPTION_TRAIL_LOCK_PRODUCTS:
                book = books.get(product)
                if not book:
                    continue
                position = int(state.position.get(product, 0))
                results[product] = self.flatten_position_orders(product, book, position)
    ...
    def update_portfolio_pnl_state(
        ...
        for product in self.POSITION_LIMITS:
            ...
            if not book:
                continue
            pnl += float(cash_by_product.get(product, 0.0)) + int(state.position.get(product, 0)) * float(book["mid"])
    ...
    def should_lock_option_trail(self, cache: Dict[str, object]) -> bool:
        peak = float(cache.get("portfolio_pnl_peak", 0.0))
        pnl = float(cache.get("portfolio_pnl", 0.0))
        return peak >= self.OPTION_TRAIL_LOCK_MIN_PEAK and peak - pnl >= self.OPTION_TRAIL_LOCK_DRAWDOWN
