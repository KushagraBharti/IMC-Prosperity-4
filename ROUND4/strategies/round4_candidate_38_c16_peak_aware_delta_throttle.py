...
class Trader:
    ...
    PEAK_THROTTLE_MIN_PEAK = 50_000.0
    PEAK_THROTTLE_MIN_DRAWDOWN = 900.0
    PEAK_THROTTLE_DRAWDOWN_FRACTION = 0.012
    PEAK_THROTTLE_DELTA_CAP = -600.0
    PEAK_THROTTLE_MOMENTUM_FLOOR = -0.25
    PEAK_THROTTLE_MAX_EDGE_SHIFT = 2.0
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            throttle = self.peak_delta_throttle(cache, net_delta, delta)
            original_position = position
            orders: List[Order] = []
            ...
            for ask_price, ask_volume in book["sell_orders"]:
                edge = fair_adj - ask_price
                if edge >= max(0.5, edge_threshold - throttle):
            ...
            for bid_price, bid_volume in book["buy_orders"]:
                edge = bid_price - fair_adj
                if edge >= edge_threshold + throttle:
                    base_size = self.OPTION_SIZE_BY_STRIKE.get(strike, self.OPTION_SIZE)
                    qty = base_size if edge < edge_threshold + 2.0 else 2 * base_size
    ...
    def update_portfolio_pnl_state(
        ...
        for product in self.POSITION_LIMITS:
            ...
            if not book:
                continue
            position = int((getattr(state, "position", {}) or {}).get(product, 0))
            pnl += float(cash_by_product.get(product, 0.0)) + position * float(book["mid"])
    ...
    def peak_delta_throttle(self, cache: Dict[str, object], net_delta: float, option_delta: float) -> float:
        if option_delta <= 0.15 or net_delta >= self.PEAK_THROTTLE_DELTA_CAP:
            return 0.0
        peak = float(cache.get("portfolio_pnl_peak", 0.0))
        pnl = float(cache.get("portfolio_pnl", 0.0))
        drawdown = peak - pnl
        if peak < self.PEAK_THROTTLE_MIN_PEAK:
            return 0.0
        if drawdown < max(self.PEAK_THROTTLE_MIN_DRAWDOWN, self.PEAK_THROTTLE_DRAWDOWN_FRACTION * peak):
            return 0.0
        if self.velvet_momentum(cache) < self.PEAK_THROTTLE_MOMENTUM_FLOOR:
            return 0.0
        return min(self.PEAK_THROTTLE_MAX_EDGE_SHIFT, (self.PEAK_THROTTLE_DELTA_CAP - net_delta) / 180.0)

    def velvet_momentum(self, cache: Dict[str, object]) -> float:
