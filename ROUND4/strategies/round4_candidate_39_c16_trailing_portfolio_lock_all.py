...
class Trader:
    ...
    PORTFOLIO_TRAIL_LOCK_MIN_PEAK = 82_000.0
    PORTFOLIO_TRAIL_LOCK_DRAWDOWN = 1_250.0
    ...
    def run(self, state: TradingState):
        ...
        if bool(cache.get("portfolio_trail_lock", False)) or self.should_lock_portfolio_trail(cache):
            cache["portfolio_trail_lock"] = True
    ...
    def should_lock_portfolio_trail(self, cache: Dict[str, object]) -> bool:
        peak = float(cache.get("portfolio_pnl_peak", 0.0))
        pnl = float(cache.get("portfolio_pnl", 0.0))
        return peak >= self.PORTFOLIO_TRAIL_LOCK_MIN_PEAK and peak - pnl >= self.PORTFOLIO_TRAIL_LOCK_DRAWDOWN
