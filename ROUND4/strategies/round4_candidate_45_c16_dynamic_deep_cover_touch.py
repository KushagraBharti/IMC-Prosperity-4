...
class Trader:
    ...
    DEEP_COVER_DELTA_TRIGGER = -850.0
    DEEP_COVER_MOMENTUM_TRIGGER = 0.65
    DEEP_COVER_SIZE = 24
    DEEP_COVER_SYMBOLS = ["VEV_4000", "VEV_4500"]
    ...
    def run(self, state: TradingState):
        ...
        self.apply_dynamic_deep_cover(results, books, state.position, velvet_fair, t_years, cache, passive=False)
        return results, 0, json.dumps(cache, separators=(",", ":"))

    def apply_dynamic_deep_cover(
        self,
        ...
        if self.velvet_momentum(cache) < self.DEEP_COVER_MOMENTUM_TRIGGER:
            return
        ...
        if net_delta > self.DEEP_COVER_DELTA_TRIGGER:
            return
        for symbol in self.DEEP_COVER_SYMBOLS:
            ...
            if position >= 0:
                continue
            qty = min(self.DEEP_COVER_SIZE, -position, max(0, -int(book["sell_orders"][0][1])))
            if qty <= 0:
                continue
            price = int(book["best_bid"]) + 1 if passive and int(book["best_bid"]) + 1 < int(book["best_ask"]) else int(book["best_ask"])
            existing = list(results.get(symbol, []))
            existing.append(Order(symbol, price, int(qty)))
