...
class Trader:
    ...
    OPTION_MARK_WEIGHTS = {
        ...
        "VEV_5400": {"Mark 14": -0.38, "Mark 01": 0.12, "Mark 22": -0.08},
        "VEV_5500": {"Mark 14": -0.25},
    }
    MARK_INVENTORY_START = 40_000
    MARK_INVENTORY_LATE_START = 60_000
    MARK_COVER_TRIGGER = 0.70
    MARK_RESHORT_TRIGGER = -1.15
    MARK_COVER_TARGET = {
        "VEV_4000": -270,
        "VEV_5200": -240,
        "VEV_5300": -220,
    ...
    def run(self, state: TradingState):
        ...
        option_mark_signals = {
            symbol: self.mark_flow_signal(cache, state, symbol, weights, f"{symbol.lower()}_mark_flow")
            for symbol, weights in self.OPTION_MARK_WEIGHTS.items()
        }
    ...
    def trade_vouchers_relative(
        ...
        t_years: float,
        timestamp: int,
        option_mark_signals: Dict[str, float],
    ) -> Dict[str, List[Order]]:
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            mark_orders, position = self.trade_mark_inventory_overlay(symbol, book, position, timestamp, option_mark_signals.get(symbol, 0.0))
    ...
    def trade_mark_inventory_overlay(
        self,
        symbol: str,
        book: Dict[str, object],
        position: int,
        ...
        orders: List[Order] = []
        if symbol not in self.MARK_COVER_TARGET or timestamp < self.MARK_INVENTORY_START:
            return orders, position

        cover_target = self.MARK_COVER_TARGET[symbol]
        if timestamp < self.MARK_INVENTORY_LATE_START and symbol != "VEV_4000":
            cover_target = min(cover_target, -270)

        if mark_signal >= self.MARK_COVER_TRIGGER and position < cover_target and book["sell_orders"]:
            ask_price, ask_volume = book["sell_orders"][0]
            capacity = cover_target - position
            qty = min(self.MARK_COVER_SIZE[symbol], capacity, max(0, -int(ask_volume)))
            if qty > 0:
                orders.append(Order(symbol, int(ask_price), int(qty)))
                position += qty
        elif mark_signal <= self.MARK_RESHORT_TRIGGER and position > -self.POSITION_LIMITS[symbol] and book["buy_orders"]:
            bid_price, bid_volume = book["buy_orders"][0]
            capacity = position + self.POSITION_LIMITS[symbol]
            qty = min(self.MARK_RESHORT_SIZE[symbol], capacity, max(0, int(bid_volume)))
            if qty > 0:
                orders.append(Order(symbol, int(bid_price), int(-qty)))
                position -= qty
