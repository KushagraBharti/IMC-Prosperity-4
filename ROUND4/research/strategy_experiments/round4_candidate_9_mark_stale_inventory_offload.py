...
class Trader:
    ...
    WEAK_OPTION_EXIT_TIMESTAMP = 999_999
    WEAK_OPTION_EXIT_SET = set()
    ...
    OPTION_STALE_MARK_WEIGHTS = {
        "VEV_4000": {"Mark 38": 0.35, "Mark 14": -0.35},
        "VEV_5200": {"Mark 22": -0.32, "Mark 14": -0.10},
        "VEV_5300": {"Mark 14": 0.32, "Mark 22": -0.18, "Mark 01": 0.12},
        "VEV_5400": {"Mark 14": -0.38, "Mark 01": 0.10, "Mark 22": -0.08},
        "VEV_5500": {"Mark 14": -0.25, "Mark 22": 0.05},
    }
    STALE_MARK_OFFLOAD_START = 86_000
    STALE_MARK_PASSIVE_TRIGGER = {
        "VEV_4000": 0.70,
        "VEV_5200": 0.20,
        "VEV_5300": 0.12,
        "VEV_5400": 0.10,
    ...
    def run(self, state: TradingState):
        ...
        option_stale_mark_signals = {
            symbol: self.mark_flow_signal(cache, state, symbol, weights, f"{symbol.lower()}_stale_mark_flow")
            for symbol, weights in self.OPTION_STALE_MARK_WEIGHTS.items()
        }
    ...
    def trade_vouchers_relative(
        ...
        t_years: float,
        timestamp: int,
        option_stale_mark_signals: Dict[str, float],
    ) -> Dict[str, List[Order]]:
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            mark_orders, position = self.trade_mark_stale_inventory_offload(
                symbol,
                book,
                position,
                timestamp,
                option_stale_mark_signals.get(symbol, 0.0),
            )
            orders.extend(mark_orders)
    ...
    def trade_mark_stale_inventory_offload(
        self,
        symbol: str,
        book: Dict[str, object],
        position: int,
        ...
        orders: List[Order] = []
        if timestamp < self.STALE_MARK_OFFLOAD_START or symbol not in self.STALE_MARK_COVER_TARGET:
            return orders, position
        if position >= self.STALE_MARK_COVER_TARGET[symbol]:
            return orders, position
        if not book["buy_orders"] or not book["sell_orders"]:
            return orders, position

        cover_capacity = self.STALE_MARK_COVER_TARGET[symbol] - position
        if cover_capacity <= 0:
            return orders, position

        if mark_signal >= self.STALE_MARK_CROSS_TRIGGER[symbol]:
            ask_price, ask_volume = book["sell_orders"][0]
            qty = min(self.STALE_MARK_CROSS_SIZE[symbol], cover_capacity, max(0, -int(ask_volume)))
            if qty > 0:
                orders.append(Order(symbol, int(ask_price), int(qty)))
                position += qty
            return orders, position

        if mark_signal >= self.STALE_MARK_PASSIVE_TRIGGER[symbol]:
            best_bid = int(book["best_bid"])
            best_ask = int(book["best_ask"])
            passive_bid = min(best_bid + 1, best_ask - 1)
            if passive_bid > best_bid and passive_bid < best_ask:
                qty = min(self.STALE_MARK_PASSIVE_SIZE[symbol], cover_capacity)
                if qty > 0:
                    orders.append(Order(symbol, int(passive_bid), int(qty)))
            return orders, position
