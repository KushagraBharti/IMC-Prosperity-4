...
class Trader:
    ...
    OPTION_MM_START_TIMESTAMP = 40_000
    OPTION_MM_SYMBOLS = {"VEV_4000", "VEV_4500", "VEV_5000", "VEV_5100", "VEV_5200", "VEV_5300", "VEV_5400", "VEV_5500"}
    OPTION_MM_SIZE = 12
    OPTION_MM_SOFT_LIMIT = 180
    OPTION_MM_FAIR_GUARD = 0.45
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            self.add_option_market_maker_orders(symbol, book, fair_adj, position, orders, timestamp)
    ...
    def add_option_market_maker_orders(
        self,
        symbol: str,
        book: Dict[str, object],
        fair_adj: float,
        ...
        if timestamp < self.OPTION_MM_START_TIMESTAMP or symbol not in self.OPTION_MM_SYMBOLS:
            return
        best_bid = int(book["best_bid"])
        best_ask = int(book["best_ask"])
        spread = best_ask - best_bid
        if spread < 2:
            return
        limit = min(self.POSITION_LIMITS[symbol], self.OPTION_MM_SOFT_LIMIT)
        size = self.OPTION_MM_SIZE
        mid = float(book["mid"])
        fair_guard = max(2.0, self.OPTION_MM_FAIR_GUARD * spread)

        if position < limit and fair_adj >= mid - fair_guard:
            qty = min(size, limit - position)
            bid = best_bid + 1
            if qty > 0 and bid < best_ask:
                orders.append(Order(symbol, bid, int(qty)))
        if position > -limit and fair_adj <= mid + fair_guard:
            qty = min(size, limit + position)
            ask = best_ask - 1
            if qty > 0 and ask > best_bid:
