...
class Trader:
    ...
    LATE_ROTATION_START = 90_000
    LATE_ROTATION_END = 92_400
    LATE_DEEP_SHORTS = {"VEV_4000", "VEV_4500", "VEV_5400", "VEV_5500"}
    LATE_WINNERS = {"VELVETFRUIT_EXTRACT", "VEV_5000", "VEV_5100"}
    ...
    def run(self, state: TradingState):
        ...
        self.apply_late_rotation(results, books, state.position, timestamp)
        return results, 0, json.dumps(cache, separators=(",", ":"))

    def apply_late_rotation(
        self,
        ...
        if self.LATE_ROTATION_START <= timestamp < self.LATE_ROTATION_END:
            for product in self.LATE_DEEP_SHORTS:
                self.set_target_inside(results, books, positions, product, 0)
        elif timestamp >= self.LATE_ROTATION_END:
            for product in self.LATE_DEEP_SHORTS:
                self.set_target_inside(results, books, positions, product, -self.POSITION_LIMITS[product])
            for product in self.LATE_WINNERS:
                self.set_target_inside(results, books, positions, product, 0)

    def set_target_inside(
        self,
        results: Dict[str, List[Order]],
        books: Dict[str, Dict[str, object]],
        positions: Dict[str, int],
        ...
        if delta > 0 and book["sell_orders"]:
            price = best_bid + 1 if best_bid + 1 < best_ask else best_ask
            results[product] = self.ensure_within_hard_limit(product, position, [Order(product, price, delta)])
        elif delta < 0 and book["buy_orders"]:
            price = best_ask - 1 if best_ask - 1 > best_bid else best_bid
            results[product] = self.ensure_within_hard_limit(product, position, [Order(product, price, delta)])
        else:
            results[product] = []
