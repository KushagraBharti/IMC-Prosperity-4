...
class Trader:
    ...
    DELTA_REBALANCE_ENABLED = True
    DELTA_REBALANCE_SHORT_TRIGGER = -750.0
    DELTA_REBALANCE_TREND_TRIGGER = 0.8
    DELTA_REBALANCE_SIZE = 60
    DELTA_REBALANCE_SYMBOLS = ["VELVETFRUIT_EXTRACT", "VEV_5000", "VEV_5100", "VEV_5200"]
    ...
    def run(self, state: TradingState):
        ...
        self.apply_dynamic_delta_rebalance(results, books, state.position, velvet_fair, t_years, cache)
        return results, 0, json.dumps(cache, separators=(",", ":"))

    def apply_dynamic_delta_rebalance(
        self,
        ...
        if not self.DELTA_REBALANCE_ENABLED:
            return
        ...
        if net_delta > self.DELTA_REBALANCE_SHORT_TRIGGER or trend < self.DELTA_REBALANCE_TREND_TRIGGER:
            return

        for symbol in self.DELTA_REBALANCE_SYMBOLS:
            ...
            qty = min(self.DELTA_REBALANCE_SIZE, capacity, max(0, -int(ask_volume)))
            if qty <= 0:
                continue
