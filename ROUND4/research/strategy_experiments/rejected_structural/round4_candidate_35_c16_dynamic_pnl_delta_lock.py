...
class Trader:
    ...
    HYDRO_TAKE_EDGE = 16.0
    HYDRO_MAKER_EDGE = 4.0
    HYDRO_QUOTE_SIZE = 72
    HYDRO_TAKE_SIZE = 80
    HYDRO_SOFT_LIMIT = 200
    HYDRO_FLATTEN_TRIGGER = 130
    HYDRO_EXIT_TIMESTAMP = 60_000
    HYDRO_PNL_LOCK_ENABLED = True
    HYDRO_PNL_LOCK_MIN_TIMESTAMP = 60_000
    HYDRO_PNL_TAKE_TIMESTAMP = 60_000
    HYDRO_PNL_TAKE_PROFIT = 1_300.0
    HYDRO_PNL_PEAK_MIN = 9_999.0
    HYDRO_PNL_MAX_DRAWDOWN = 9_999.0
    HYDRO_MARK_FAIR_WEIGHT = 1.5
    HYDRO_MARK_EXEC_THRESHOLD = 0.8
    HYDRO_MARK_EXEC_SIZE = 48
    HYDRO_PASSIVE_MARK_THRESHOLD = 0.15
    HYDRO_PASSIVE_MARK_EDGE = 2.0
    HYDRO_PASSIVE_MARK_SIZE = 36
    ...
    VELVET_REENTRY_TIMESTAMP = 85000
    TIMED_OPTION_EXIT = {"VEV_5000": 71_900}
    TIMED_OPTION_REENTRY = {"VEV_5000": 85000}
    ...
    PORTFOLIO_LOCK_MIN_PEAK = 35_000.0
    PORTFOLIO_LOCK_MIN_DRAWDOWN = 1_000.0
    PORTFOLIO_LOCK_DRAWDOWN_FRACTION = 0.015
    PORTFOLIO_LOCK_SHORT_DELTA_TRIGGER = -600.0
    PORTFOLIO_LOCK_TREND_FLOOR = -0.5
    PORTFOLIO_LOCK_HEDGE_SIZE = 80
    PORTFOLIO_LOCK_HEDGE_SYMBOLS = ["VELVETFRUIT_EXTRACT", "VEV_5000", "VEV_5100", "VEV_5200"]
    ...
    def run(self, state: TradingState):
        ...
        if hydro_book and hydro_book["has_both_sides"]:
            ...
            self.update_hydro_pnl_state(cache, state, hydro_book, hydro_pos)
            hydro_locked = bool(cache.get("hydro_lock", False)) or self.should_lock_hydro(cache, timestamp)
            if hydro_locked:
                cache["hydro_lock"] = True
            if timestamp >= self.TRADE_START_TIMESTAMP:
                if timestamp >= self.HYDRO_EXIT_TIMESTAMP or hydro_locked:
        ...
        self.update_portfolio_pnl_state(cache, state, books)
        self.apply_dynamic_pnl_delta_lock(results, books, state.position, velvet_fair, t_years, cache)
        return results, 0, json.dumps(cache, separators=(",", ":"))

    def update_portfolio_pnl_state(
        self,
        cache: Dict[str, object],
        state: TradingState,
        books: Dict[str, Dict[str, object]],
    ) -> None:
        cash_by_product = cache.get("portfolio_cash")
        if not isinstance(cash_by_product, dict):
            cash_by_product = {}
        seen_trades = cache.get("portfolio_seen_trades")
        if not isinstance(seen_trades, list):
            seen_trades = []
        seen_lookup = set(str(x) for x in seen_trades)

        for product, trades in (getattr(state, "own_trades", {}) or {}).items():
            cash = float(cash_by_product.get(product, 0.0))
            for trade in trades:
                price = float(self.trade_attr(trade, "price", 0.0))
                quantity = float(self.trade_attr(trade, "quantity", 0.0))
                buyer = self.trade_attr(trade, "buyer", "")
                seller = self.trade_attr(trade, "seller", "")
                trade_key = f"{product}:{self.trade_attr(trade, 'timestamp', 0)}:{price}:{quantity}:{buyer}:{seller}"
                if trade_key in seen_lookup:
                    continue
                seen_lookup.add(trade_key)
                seen_trades.append(trade_key)
                if buyer == "SUBMISSION":
                    cash -= price * quantity
                elif seller == "SUBMISSION":
                    cash += price * quantity
            cash_by_product[product] = cash
        ...
        for product, limit in self.POSITION_LIMITS.items():
            ...
            if not book:
                continue
            pnl += float(cash_by_product.get(product, 0.0)) + float((getattr(state, "position", {}) or {}).get(product, 0)) * float(book["mid"])

        peak = max(float(cache.get("portfolio_pnl_peak", pnl)), pnl)
        cache["portfolio_cash"] = cash_by_product
        cache["portfolio_seen_trades"] = seen_trades[-400:]
        cache["portfolio_pnl"] = pnl
        cache["portfolio_pnl_peak"] = peak

    def apply_dynamic_pnl_delta_lock(
        self,
        ...
        if not self.PORTFOLIO_LOCK_ENABLED:
            return
        peak = float(cache.get("portfolio_pnl_peak", 0.0))
        pnl = float(cache.get("portfolio_pnl", 0.0))
        drawdown = peak - pnl
        if peak < self.PORTFOLIO_LOCK_MIN_PEAK:
            return
        if drawdown < max(self.PORTFOLIO_LOCK_MIN_DRAWDOWN, self.PORTFOLIO_LOCK_DRAWDOWN_FRACTION * peak):
            return

        history = cache.get("velvet_mid", [])
        if not isinstance(history, list) or len(history) < 12:
            return
        short_avg = sum(float(x) for x in history[-4:]) / 4.0
        long_avg = sum(float(x) for x in history[-16:]) / min(len(history), 16)
        trend = short_avg - long_avg

        net_delta = float(positions.get("VELVETFRUIT_EXTRACT", 0))
        for symbol in self.ACTIVE_VOUCHERS:
            strike = self.STRIKES[symbol]
            sigma = self.STRIKE_VOL[strike]
            net_delta += float(positions.get(symbol, 0)) * self.black_scholes_delta(spot_fair, strike, t_years, sigma)

        if net_delta > self.PORTFOLIO_LOCK_SHORT_DELTA_TRIGGER or trend < self.PORTFOLIO_LOCK_TREND_FLOOR:
            return

        for symbol in self.PORTFOLIO_LOCK_HEDGE_SYMBOLS:
            book = books.get(symbol)
            if not book or not book["sell_orders"]:
                continue
            position = int(positions.get(symbol, 0))
            capacity = self.POSITION_LIMITS[symbol] - position
            if capacity <= 0:
                continue
            ask_price, ask_volume = book["sell_orders"][0]
            qty = min(self.PORTFOLIO_LOCK_HEDGE_SIZE, capacity, max(0, -int(ask_volume)))
            if qty <= 0:
                continue
            existing = list(results.get(symbol, []))
            existing.append(Order(symbol, int(ask_price), int(qty)))
            results[symbol] = self.ensure_within_hard_limit(symbol, position, existing)

    def update_hydro_pnl_state(self, cache: Dict[str, object], state: TradingState, book: Dict[str, object], position: int) -> None:
        cash = float(cache.get("hydro_cash", 0.0))
        previous_position = int(cache.get("hydro_last_pos", 0))
        position_delta = position - previous_position
        seen_trades = list(cache.get("hydro_seen_trades", []))
        seen_lookup = set(seen_trades)
        for trade in (getattr(state, "own_trades", {}) or {}).get("HYDROGEL_PACK", []):
            price = float(self.trade_attr(trade, "price", 0))
            quantity = float(self.trade_attr(trade, "quantity", 0))
            buyer = self.trade_attr(trade, "buyer", "")
            seller = self.trade_attr(trade, "seller", "")
            trade_key = f"{self.trade_attr(trade, 'timestamp', 0)}:{price}:{quantity}:{buyer}:{seller}"
            if trade_key in seen_lookup:
                continue
            seen_lookup.add(trade_key)
            seen_trades.append(trade_key)
            if buyer == "SUBMISSION":
                ...
                signed_quantity = quantity if position_delta > 0 else -quantity
                cash -= price * signed_quantity
        pnl = cash + float(position) * float(book["mid"])
        peak = max(float(cache.get("hydro_pnl_peak", pnl)), pnl)
        cache["hydro_cash"] = cash
        cache["hydro_pnl"] = pnl
        cache["hydro_pnl_peak"] = peak
        cache["hydro_last_pos"] = position
        cache["hydro_seen_trades"] = seen_trades[-80:]

    def should_lock_hydro(self, cache: Dict[str, object], timestamp: int) -> bool:
        if not self.HYDRO_PNL_LOCK_ENABLED or timestamp < self.HYDRO_PNL_LOCK_MIN_TIMESTAMP:
            return False
        pnl = float(cache.get("hydro_pnl", 0.0))
        peak = float(cache.get("hydro_pnl_peak", pnl))
        if timestamp >= self.HYDRO_PNL_TAKE_TIMESTAMP and pnl >= self.HYDRO_PNL_TAKE_PROFIT:
            return True
        return peak >= self.HYDRO_PNL_PEAK_MIN and peak - pnl >= self.HYDRO_PNL_MAX_DRAWDOWN
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            if timed_exit is not None and timed_exit <= timestamp < self.TIMED_OPTION_REENTRY.get(symbol, 999_999):
                if position < 0 and book["sell_orders"]:
