...
class Trader:
    ...
    OPTION_PASSIVE_ENABLED = True
    OPTION_PASSIVE_START_TIMESTAMP = 40_000
    OPTION_PASSIVE_EDGE_MULT = 0.80
    OPTION_PASSIVE_SIZE = 8
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            if self.OPTION_PASSIVE_ENABLED and timestamp >= self.OPTION_PASSIVE_START_TIMESTAMP:
                passive_edge = max(0.5, edge_threshold * self.OPTION_PASSIVE_EDGE_MULT)
                best_bid = int(book["best_bid"])
                best_ask = int(book["best_ask"])
                if best_ask > best_bid:
                    if fair_adj - float(book["mid"]) >= passive_edge and position < self.POSITION_LIMITS[symbol]:
                        price = min(best_bid + 1, int(math.floor(fair_adj - passive_edge)))
                        qty = min(self.OPTION_PASSIVE_SIZE, self.POSITION_LIMITS[symbol] - position)
                        if qty > 0 and price < best_ask:
                            orders.append(Order(symbol, price, int(qty)))
                            position += qty
                    elif float(book["mid"]) - fair_adj >= passive_edge and position > -self.POSITION_LIMITS[symbol]:
                        price = max(best_ask - 1, int(math.ceil(fair_adj + passive_edge)))
                        qty = min(self.OPTION_PASSIVE_SIZE, self.POSITION_LIMITS[symbol] + position)
