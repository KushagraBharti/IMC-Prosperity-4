...
class Trader:
    ...
    MARK_COVER_START = 83_000
    MARK_COVER_TRIGGER = 1.25
    MARK_COVER_TARGET = {
        "VEV_5200": -250,
        "VEV_5300": -245,
        "VEV_5400": -240,
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            mark_orders, position = self.trade_late_mark_cover(symbol, book, position, timestamp, option_mark_signals.get(symbol, 0.0))
    ...
    def trade_late_mark_cover(
        ...
        orders: List[Order] = []
        if timestamp < self.MARK_COVER_START or symbol not in self.MARK_COVER_TARGET:
            return orders, position
        if mark_signal < self.MARK_COVER_TRIGGER or position >= self.MARK_COVER_TARGET[symbol]:
            return orders, position
        if not book["sell_orders"]:
            return orders, position

        ask_price, ask_volume = book["sell_orders"][0]
        capacity = self.MARK_COVER_TARGET[symbol] - position
