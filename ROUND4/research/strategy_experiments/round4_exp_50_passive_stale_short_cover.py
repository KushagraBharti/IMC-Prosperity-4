...
class Trader:
    ...
    PASSIVE_SHORT_COVER_TIMESTAMP = {"VEV_4000": 87_300, "VEV_4500": 87_300, "VEV_5300": 43_000, "VEV_5400": 43_000}
    PASSIVE_SHORT_COVER_SIZE = 60
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            passive_cover_at = self.PASSIVE_SHORT_COVER_TIMESTAMP.get(symbol)
            if passive_cover_at is not None and timestamp >= passive_cover_at and position < 0:
                price = min(int(book["best_ask"]) - 1, int(book["best_bid"]) + 1)
                qty = min(-position, self.PASSIVE_SHORT_COVER_SIZE)
                if qty > 0 and price < int(book["best_ask"]):
