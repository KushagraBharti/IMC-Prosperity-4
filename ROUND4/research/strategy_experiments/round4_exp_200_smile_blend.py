...
class Trader:
    ...
    HYDRO_PASSIVE_MARK_THRESHOLD = 0.5
    HYDRO_PASSIVE_MARK_EDGE = 2.0
    HYDRO_PASSIVE_MARK_SIZE = 18
    ...
    def trade_vouchers_relative(
        ...
        smile_fairs = self.compute_leave_one_out_smile_fairs(books, spot_fair, t_years)
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            strike = self.STRIKES[symbol]
            sigma = self.STRIKE_VOL[strike]
            fixed_fair = self.black_scholes_call(spot_fair, strike, t_years, sigma)
            smile_fair = smile_fairs.get(symbol, fixed_fair)
            fair = (1.0 - self.OPTION_SMILE_BLEND) * fixed_fair + self.OPTION_SMILE_BLEND * smile_fair
    ...
    def compute_leave_one_out_smile_fairs(
        self,
        books: Dict[str, Dict[str, object]],
        spot_fair: float,
        t_years: float,
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            if not book or not book["has_both_sides"]:
                continue
            strike = self.STRIKES[symbol]
            iv = self.implied_vol(float(book["mid"]), spot_fair, strike, t_years)
            if iv is None:
                continue
            m = math.log(strike / spot_fair) / max(math.sqrt(t_years), 1e-6)
            observations.append((symbol, m, iv))

        fairs: Dict[str, float] = {}
        if len(observations) < 4:
            return fairs
        for target, _, _ in observations:
            sample = [(m, iv) for symbol, m, iv in observations if symbol != target]
            if len(sample) < 3:
                continue
            a, b, c = self.fit_quadratic(sample)
            strike = self.STRIKES[target]
            m = math.log(strike / spot_fair) / max(math.sqrt(t_years), 1e-6)
            vol = max(0.05, min(0.90, a * m * m + b * m + c))
            fairs[target] = self.black_scholes_call(spot_fair, strike, t_years, vol)
        return fairs
