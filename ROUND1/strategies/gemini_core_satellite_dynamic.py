...
class Trader:
    ...
    # OSMIUM: The portal-validated 224169 specialist.
    OSMIUM_FAIR = 10000.0
    OSMIUM_BASE_SIZE = 20
    ...
    # PEPPER: Gemini Dynamic Core-Satellite Strategy
    # Core captures structural drift. Satellite actively trades large deviations dynamically.
    # Non-hardcoded state transitions prevent overfitting to early ticks.
    PEPPER_TREND_PER_TIMESTAMP = 0.001
    PEPPER_DEFAULT_ANCHOR = 13000.0
    PEPPER_ANCHOR_SMOOTHING = 0.10

    PEPPER_FORWARD_PREMIUM = 8.0
    PEPPER_DEV_WEIGHT = 1.15
    PEPPER_IMBALANCE_WEIGHT = 2.80
    PEPPER_TREND_WEIGHT = 0.40
    PEPPER_IMBALANCE_TREND_WEIGHT = 0.35
    PEPPER_MID_TREND_WEIGHT = 0.15
    PEPPER_ALPHA_CLIP = 4.5

    PEPPER_CORE_BASE = 58
    PEPPER_CORE_SLOPE = 4
    PEPPER_CORE_CLIP = 2.5
    PEPPER_CORE_IMB_BONUS = 5
    PEPPER_CORE_MIN = 50
    PEPPER_CORE_MAX = 70

    PEPPER_SAT_BUILD = 18
    PEPPER_SAT_HOLD = 14
    PEPPER_MIN_SAT_TO_TRIM = 8

    # Transition Thresholds based on residuals and alpha
    PEPPER_TRIM_RESIDUAL = 4.0
    PEPPER_RELOAD_RESIDUAL = 1.0
    PEPPER_TRIM_BIAS = 1.2
    PEPPER_TRIM_ALPHA = 0.25
    PEPPER_TRIM_EDGE = 2.0

    PEPPER_SWEEP_LEVELS = 3
    PEPPER_TAKE_EDGE = 0.5
    PEPPER_CATCHUP_EDGE_SMALL = -0.25
    PEPPER_CATCHUP_EDGE_LARGE = -0.75
    PEPPER_LARGE_GAP = 10

    PEPPER_BUY_BASE = 4
    PEPPER_BUY_GAP_CAP = 12
    PEPPER_STRONG_BUY_EDGE = 1.5
    PEPPER_STRONG_BUY_BONUS = 5

    PEPPER_SELL_BASE = 3
    PEPPER_SELL_EXCESS_CAP = 8

    PEPPER_PASSIVE_GAP_TRIGGER = 8
    ...
    def trade_pepper_core_satellite(
        ...
        # Passive bid when building toward core or satellite.
        if buy_capacity > 0 and gap_to_target >= self.PEPPER_PASSIVE_GAP_TRIGGER and mode != "TRIM":
            ...
            if bid_quote < best_ask:
                bid_size = min(buy_capacity, max(3, min(gap_to_target, 8)))
                if bid_size > 0:
