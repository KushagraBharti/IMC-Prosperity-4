...
class Trader:
    ...
    # PEPPER: Gemini V3 Aggressive Dynamic Accumulator.
    # Completely decoupled from time/price scripts. Pure fair-value modeling.
    PEPPER_TREND_PER_TIMESTAMP = 0.001
    PEPPER_DEFAULT_ANCHOR = 13000.0
    PEPPER_ANCHOR_SMOOTHING = 0.10

    PEPPER_FORWARD_PREMIUM = 8.5
    PEPPER_DEV_WEIGHT = 1.20
    PEPPER_IMBALANCE_WEIGHT = 3.00
    PEPPER_TREND_WEIGHT = 0.45
    PEPPER_IMBALANCE_TREND_WEIGHT = 0.40
    PEPPER_MID_TREND_WEIGHT = 0.20
    PEPPER_ALPHA_CLIP = 4.5

    PEPPER_BASE_TARGET = 78
    PEPPER_TARGET_SLOPE = 0
    PEPPER_TARGET_CLIP = 3.0
    ...
    PEPPER_CATCHUP_EDGE_SMALL = -0.50
    PEPPER_CATCHUP_EDGE_LARGE = -1.25
    PEPPER_LARGE_GAP = 12
    ...
    # Dynamic Micro-trim for extreme book dislocations
    PEPPER_TRIM_RESIDUAL = 4.5
    PEPPER_TRIM_BIAS = 1.2
    PEPPER_TRIM_ALPHA = 0.15
    PEPPER_TRIM_EDGE = 2.5
    PEPPER_TRIM_FLOOR = 75
    PEPPER_TRIM_SIZE = 5
    PEPPER_SELL_DEV_THRESH = -0.8
    PEPPER_SELL_IMB_THRESH = -0.2
    ...
    def run(self, state: TradingState):
        ...
        if pepper_depth is not None:
            ...
            result["INTARIAN_PEPPER_ROOT"] = self.trade_pepper(
                pepper_book,
                pepper_position,
                timestamp,
                cache,
    ...
    def trade_pepper(
        ...
        # Extremely selective aggressive trim. No passive pepper offers.
        # Trims to capture rich spikes and reload later.
        if (
            (not did_buy)
            and position >= self.PEPPER_TRIM_FLOOR + self.PEPPER_TRIM_SIZE
            and residual >= self.PEPPER_TRIM_RESIDUAL
            and deviation <= self.PEPPER_SELL_DEV_THRESH
            and book["imbalance"] <= self.PEPPER_SELL_IMB_THRESH
        ):
