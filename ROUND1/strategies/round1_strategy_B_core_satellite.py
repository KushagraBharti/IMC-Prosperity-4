...
class Trader:
    ...
    # Portal-proven osmium specialist kept intact from 224169 / 233545 / 233714.
    OSMIUM_FAIR = 10000.0
    OSMIUM_BASE_SIZE = 20
    ...
    # Pepper: core-satellite state machine.
    # Core stays long almost all the time.
    # Satellite trims only when the detrended residual is genuinely rich,
    # and reloads when the residual normalizes again.
    PEPPER_TREND_PER_TIMESTAMP = 0.001
    PEPPER_DEFAULT_ANCHOR = 13000.0
    PEPPER_ANCHOR_SMOOTHING = 0.10

    PEPPER_FORWARD_PREMIUM = 7.0
    PEPPER_DEV_WEIGHT = 1.00
    PEPPER_IMBALANCE_WEIGHT = 2.60
    PEPPER_TREND_WEIGHT = 0.35
    PEPPER_IMBALANCE_TREND_WEIGHT = 0.30
    PEPPER_MID_TREND_WEIGHT = 0.10
    PEPPER_ALPHA_CLIP = 4.0

    PEPPER_CORE_BASE = 58
    PEPPER_CORE_SLOPE = 4
    PEPPER_CORE_CLIP = 2.0
    PEPPER_CORE_IMB_BONUS = 4
    PEPPER_CORE_MIN = 52
    PEPPER_CORE_MAX = 68

    PEPPER_SAT_BUILD = 16
    PEPPER_SAT_HOLD = 12
    PEPPER_MIN_SAT_TO_TRIM = 6

    PEPPER_TRIM_RESIDUAL = 4.5
    PEPPER_RELOAD_RESIDUAL = 1.0
    PEPPER_TRIM_BIAS = 1.0
    PEPPER_TRIM_ALPHA = 0.20
    PEPPER_TRIM_EDGE = 2.0

    PEPPER_SWEEP_LEVELS = 3
    PEPPER_TAKE_EDGE = 0.5
    PEPPER_CATCHUP_EDGE_SMALL = -0.10
    PEPPER_CATCHUP_EDGE_LARGE = -0.35
    PEPPER_LARGE_GAP = 10
    ...
    PEPPER_SELL_BASE = 2
    PEPPER_SELL_EXCESS_CAP = 6

    PEPPER_PASSIVE_GAP_TRIGGER = 6
    PEPPER_PASSIVE_BID_IMPROVE = 2
    PEPPER_PASSIVE_ASK_IMPROVE = 1
    ...
    def run(self, state: TradingState):
        ...
        if pepper_depth is not None:
            ...
            result["INTARIAN_PEPPER_ROOT"] = self.trade_pepper_core_satellite(
                pepper_book,
                pepper_position,
                timestamp,
                cache,
    ...
    def trade_pepper_core_satellite(
        self,
        book: Dict[str, object],
        position: int,
        timestamp: int,
        ...
        core_target = (
            self.PEPPER_CORE_BASE
            + int(round(self.clip(alpha, -self.PEPPER_CORE_CLIP, self.PEPPER_CORE_CLIP) * self.PEPPER_CORE_SLOPE))
            + int(round(max(0.0, book["imbalance"]) * self.PEPPER_CORE_IMB_BONUS))
        )
        core_target = int(self.clip(core_target, self.PEPPER_CORE_MIN, self.PEPPER_CORE_MAX))

        mode = str(cache.get("pepper_mode", "BUILD"))
        if mode == "TRIM":
            if residual <= self.PEPPER_RELOAD_RESIDUAL or position <= core_target:
                mode = "BUILD"
        else:
            if residual >= self.PEPPER_TRIM_RESIDUAL and position > core_target + self.PEPPER_MIN_SAT_TO_TRIM:
                mode = "TRIM"
            elif position < core_target:
                mode = "BUILD"
            elif position >= core_target + self.PEPPER_SAT_BUILD // 2:
                mode = "HOLD"

        if mode == "BUILD":
            target_position = min(limit, core_target + self.PEPPER_SAT_BUILD)
        elif mode == "TRIM":
            target_position = core_target
        else:
            target_position = min(limit, core_target + self.PEPPER_SAT_HOLD)

        cache["pepper_mode"] = mode
        ...
        # Small passive bid when still building toward core or satellite.
        if buy_capacity > 0 and gap_to_target >= self.PEPPER_PASSIVE_GAP_TRIGGER and mode != "TRIM":
            ...
            if bid_quote < best_ask:
                bid_size = min(buy_capacity, max(3, min(gap_to_target, 6)))
                if bid_size > 0:
        ...
        for level, (bid_price, bid_volume) in enumerate(book["buy_orders"]):
            if level >= 2 or sell_capacity <= 0 or excess <= 0:
                break
            available = max(0, bid_volume)
            if available <= 0:
                continue
        ...
        # Small passive ask only in explicit trim mode, and never through the bid.
        if sell_capacity > 0 and excess > 0 and mode == "TRIM":
            best_bid = int(book["best_bid"])
            best_ask = int(book["best_ask"])
            ask_quote = max(
                best_ask - self.PEPPER_PASSIVE_ASK_IMPROVE,
                int(math.ceil(trim_fair + self.PEPPER_PASSIVE_MAKER_EDGE)),
            )
            if ask_quote > best_bid:
                ask_size = min(sell_capacity, excess, max(3, min(excess, 6)))
                if ask_size > 0:
    ...
    def pepper_sell_size(self, excess: int) -> int:
        return max(2, self.PEPPER_SELL_BASE + min(self.PEPPER_SELL_EXCESS_CAP, excess))
