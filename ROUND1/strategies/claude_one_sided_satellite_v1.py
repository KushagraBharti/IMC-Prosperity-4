...
class Trader:
    ...
    PEPPER_ONE_SIDED_EDGE_SMALL = 0.15
    PEPPER_ONE_SIDED_EDGE_LARGE = 0.00
    PEPPER_LEVEL_TWO_MIN_EDGE = 0.40
    PEPPER_LARGE_GAP = 12
    PEPPER_PRESSURE_GAP_TRIGGER = 18
    PEPPER_PRESSURE_PREMIUM_BONUS = 0.45
    PEPPER_PRESSURE_SIZE_BONUS = 2

    PEPPER_BUY_BASE = 4
    PEPPER_BUY_GAP_CAP = 10
    PEPPER_STRONG_BUY_EDGE = 1.5
    PEPPER_STRONG_BUY_BONUS = 4
    PEPPER_ONE_SIDED_GAP_BONUS = 2

    PEPPER_TRIM_WINDOW = 60
    PEPPER_TRIM_MIN_SAMPLES = 25
    PEPPER_TRIM_Z = 2.2
    PEPPER_RELOAD_Z = 0.5
    PEPPER_TRIM_SIZE = 2
    PEPPER_TRIM_EDGE = 2.5
    PEPPER_TRIM_FLOOR = 76
    PEPPER_TRIM_IMB = -0.20
    PEPPER_TRIM_DEV = -0.60
    PEPPER_TRIM_COOLDOWN = 35
    PEPPER_TRIM_TIMEOUT = 60
    PEPPER_TRIM_BIAS = 1.4
    PEPPER_TRIM_ALPHA = 0.10
    PEPPER_STD_FLOOR = 0.75
    ...
    def run(self, state: TradingState):
        ...
        if pepper_depth is not None:
            ...
            if pepper_book["has_both_sides"]:
                ...
                cache["pepper_last_mid"] = pepper_book["mid"]
            ...
            result["INTARIAN_PEPPER_ROOT"] = self.trade_pepper_satellite(
                pepper_book,
                pepper_position,
                timestamp,
                cache,
    ...
    def trade_pepper_satellite(
        ...
        if not book["sell_orders"] and not book["buy_orders"]:
            return []

        has_both_sides = bool(book["buy_orders"]) and bool(book["sell_orders"])
        ...
        last_mid = float(cache.get("pepper_last_mid", base_fair))
        synthetic_mid = book["mid"] if has_both_sides else last_mid
        ...
        deviation = book["wall_mid"] - book["mid"] if has_both_sides else 0.0
        imbalance = book["imbalance"] if has_both_sides else 0.0
        alpha = (
            deviation * self.PEPPER_DEV_WEIGHT
            + imbalance * self.PEPPER_IMBALANCE_WEIGHT
        ...
        if not has_both_sides and book["sell_orders"]:
            alpha += self.clip((synthetic_mid - book["best_ask"]) * self.PEPPER_ONE_SIDED_EDGE_WEIGHT, -0.5, 1.0)
        alpha = self.clip(alpha, -self.PEPPER_ALPHA_CLIP, self.PEPPER_ALPHA_CLIP)

        residual = synthetic_mid - base_fair
        raw_target = (
            ...
            + int(round(max(0.0, imbalance) * self.PEPPER_IMB_TARGET_BONUS))
        )
        target_position = int(self.clip(raw_target, self.PEPPER_MIN_TARGET, self.PEPPER_MAX_TARGET))

        residual_history = cache.get("pepper_residual", [])
        residual_std = self.rolling_std(residual_history, self.PEPPER_TRIM_WINDOW)
        residual_z = residual / max(self.PEPPER_STD_FLOOR, residual_std)

        trimmed_qty = int(cache.get("pepper_trimmed_qty", 0))
        last_trim_ts = int(cache.get("pepper_last_trim_ts", -1_000_000))
        if trimmed_qty > 0 and (residual_z <= self.PEPPER_RELOAD_Z or timestamp - last_trim_ts >= self.PEPPER_TRIM_TIMEOUT):
            trimmed_qty = 0

        effective_target = max(self.PEPPER_TRIM_FLOOR, target_position - trimmed_qty)
        initial_gap_to_target = max(0, effective_target - position)
        pressure_bonus = self.PEPPER_PRESSURE_PREMIUM_BONUS if initial_gap_to_target >= self.PEPPER_PRESSURE_GAP_TRIGGER else 0.0
        forward_fair = base_fair + self.PEPPER_FORWARD_PREMIUM + alpha + pressure_bonus
        trim_fair = base_fair + self.PEPPER_TRIM_BIAS + self.PEPPER_TRIM_ALPHA * alpha

        if book["sell_orders"]:
            buy_capacity = max(0, limit - position)
            gap_to_target = max(0, effective_target - position)
            max_levels = 1
            if has_both_sides and gap_to_target >= self.PEPPER_LARGE_GAP:
                max_levels = self.PEPPER_SWEEP_LEVELS

            for level, (ask_price, ask_volume) in enumerate(book["sell_orders"]):
                if level >= max_levels or buy_capacity <= 0:
                    break
                available = max(0, -ask_volume)
                if available <= 0:
                    continue
                ...
                if has_both_sides:
                    ...
                    catchup_edge = (
                        self.PEPPER_ONE_SIDED_EDGE_LARGE
                        if gap_to_target >= self.PEPPER_LARGE_GAP
                        else self.PEPPER_ONE_SIDED_EDGE_SMALL
                    )

                min_edge = self.PEPPER_LEVEL_TWO_MIN_EDGE if level == 1 else catchup_edge
                if level == 0:
                    should_buy = edge >= self.PEPPER_TAKE_EDGE or (gap_to_target > 0 and edge >= catchup_edge)
                else:
                    should_buy = gap_to_target > 0 and edge >= min_edge

                if should_buy:
                    size = min(
                        buy_capacity,
                        available,
                        self.pepper_buy_size(gap_to_target, edge, has_both_sides),
                    )
                    if size > 0:
                        orders.append(Order(product, int(ask_price), int(size)))
                        buy_capacity -= size
        ...
        can_trim = (
            has_both_sides
            and trimmed_qty == 0
            and len(residual_history) >= self.PEPPER_TRIM_MIN_SAMPLES
            and timestamp - last_trim_ts >= self.PEPPER_TRIM_COOLDOWN
            and position >= self.PEPPER_TRIM_FLOOR + self.PEPPER_TRIM_SIZE
            and residual_z >= self.PEPPER_TRIM_Z
            and imbalance <= self.PEPPER_TRIM_IMB
            and deviation <= self.PEPPER_TRIM_DEV
        )
        if can_trim:
            remaining = self.PEPPER_TRIM_SIZE
            for level, (bid_price, bid_volume) in enumerate(book["buy_orders"]):
                if level >= 2 or remaining <= 0:
                    break
                available = max(0, bid_volume)
                if available <= 0:
                    continue
                edge = bid_price - trim_fair
                if edge >= self.PEPPER_TRIM_EDGE:
                    size = min(remaining, available, max(0, position - self.PEPPER_TRIM_FLOOR))
                    if size > 0:
                        orders.append(Order(product, int(bid_price), int(-size)))
                        remaining -= size
                        position -= size
            sold = self.PEPPER_TRIM_SIZE - remaining
            if sold > 0:
                cache["pepper_trimmed_qty"] = sold
                cache["pepper_last_trim_ts"] = timestamp

        self.push_history(cache, "pepper_residual", residual)
        return self.enforce_long_only_limit(limit, original_position, orders)

    def pepper_buy_size(self, gap_to_target: int, edge: float, has_both_sides: bool) -> int:
        ...
        if gap_to_target >= self.PEPPER_PRESSURE_GAP_TRIGGER:
            size += self.PEPPER_PRESSURE_SIZE_BONUS
        return max(2, size)

    def rolling_std(self, history: List[float], window: int) -> float:
        if len(history) < 2:
            return 0.0
