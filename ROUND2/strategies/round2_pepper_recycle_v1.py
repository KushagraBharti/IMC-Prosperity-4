...
class Trader:
    ...
    # Keep a small, non-disruptive MAF. Portal testing ignores it anyway.
    ...
    PEPPER_FORWARD_PREMIUM = 7.75
    PEPPER_DEV_WEIGHT = 1.00
    PEPPER_IMBALANCE_WEIGHT = 2.60
    PEPPER_TREND_WEIGHT = 0.35
    PEPPER_IMBALANCE_TREND_WEIGHT = 0.30
    PEPPER_MID_TREND_WEIGHT = 0.15
    PEPPER_ALPHA_CLIP = 4.0

    PEPPER_BASE_TARGET = 72
    PEPPER_MIN_TARGET = 58
    PEPPER_MAX_TARGET = 80

    PEPPER_SWEEP_LEVELS = 2
    PEPPER_BUY_BASE = 6
    PEPPER_BUY_GAP_CAP = 10
    PEPPER_STRONG_BUY_EDGE = 1.25
    PEPPER_STRONG_BUY_BONUS = 4
    PEPPER_LARGE_GAP = 14
    PEPPER_CATCHUP_EDGE_SMALL = -0.20
    PEPPER_CATCHUP_EDGE_LARGE = -0.85

    PEPPER_PASSIVE_TRIGGER_GAP = 8
    PEPPER_PASSIVE_BID_IMPROVE = 1
    PEPPER_PASSIVE_MAKER_EDGE = 1.00
    PEPPER_PASSIVE_MAX_SIZE = 8

    PEPPER_SELL_TRIGGER_RESIDUAL = 2.75
    PEPPER_SELL_STRONG_RESIDUAL = 4.50
    PEPPER_SELL_MAX_SIZE = 10
    PEPPER_SELL_LEVELS = 2
    ...
    def run(self, state: TradingState):
        ...
        if pepper_depth is not None:
            ...
            result["INTARIAN_PEPPER_ROOT"] = self.trade_pepper_recycler(
                pepper_book,
                pepper_position,
                timestamp,
                cache,
    ...
    def trade_osmium(
        ...
        if buy_capacity > 0 and best_bid < best_ask:
            orders.append(Order(product, int(best_bid), int(min(self.OSMIUM_SECONDARY_SIZE, buy_capacity))))

        if sell_capacity > 0 and best_ask > best_bid:
            orders.append(Order(product, int(best_ask), int(-min(self.OSMIUM_SECONDARY_SIZE, sell_capacity))))

        return self.ensure_within_limits(product, original_position, orders)

    # ----- PEPPER -----
    def trade_pepper_recycler(
        self,
        book: Dict[str, object],
        position: int,
        timestamp: int,
        ...
        target_position = self.PEPPER_BASE_TARGET
        if residual < -2.5:
            target_position += 4
        elif residual > 3.0:
            target_position -= 8

        if residual > self.PEPPER_SELL_STRONG_RESIDUAL and book["imbalance"] < -0.20:
            target_position -= 6

        if book["imbalance"] > 0.35:
            target_position += 3
        elif book["imbalance"] < -0.35:
            target_position -= 4
        ...
        target_position = int(self.clip(target_position, self.PEPPER_MIN_TARGET, self.PEPPER_MAX_TARGET))

        # 1) Monetize rich states first if we are already plenty long.
        desired_reduction = 0
        if position > target_position:
            desired_reduction = position - target_position
        elif position >= 74 and residual > self.PEPPER_SELL_TRIGGER_RESIDUAL and book["imbalance"] < -0.15:
            desired_reduction = min(position - 68, 8)

        if desired_reduction > 0:
            exit_fair = base_fair + 5.5 + 0.35 * alpha
            for level, (bid_price, bid_volume) in enumerate(book["buy_orders"]):
                if level >= self.PEPPER_SELL_LEVELS or desired_reduction <= 0:
                    break
                available = max(0, bid_volume)
                if available <= 0:
                    continue
                rich_enough = bid_price >= exit_fair
                panic_trim = residual > self.PEPPER_SELL_STRONG_RESIDUAL and book["imbalance"] < -0.35
                if rich_enough or panic_trim:
                    size = min(available, desired_reduction, self.PEPPER_SELL_MAX_SIZE)
                    if size > 0:
                        orders.append(Order(product, int(bid_price), int(-size)))
                        position -= size
                        desired_reduction -= size

        # 2) Rebuild / add to the core carry position.
        buy_capacity = max(0, limit - position)
        gap_to_target = max(0, target_position - position)

        for level, (ask_price, ask_volume) in enumerate(book["sell_orders"]):
            if level >= self.PEPPER_SWEEP_LEVELS or buy_capacity <= 0 or gap_to_target <= 0:
                break
            available = max(0, -ask_volume)
            if available <= 0:
                continue
            ...
            catchup_edge = (
                self.PEPPER_CATCHUP_EDGE_LARGE if gap_to_target >= self.PEPPER_LARGE_GAP else self.PEPPER_CATCHUP_EDGE_SMALL
            )

            if edge >= 0.0 or edge >= catchup_edge:
                size = min(buy_capacity, available, self.pepper_buy_size(gap_to_target, edge))
                if size > 0:
                    orders.append(Order(product, int(ask_price), int(size)))
                    buy_capacity -= size
                    position += size
        ...
        # 3) Passive Pepper bid when still below target and not already chasing.
        buy_capacity = max(0, limit - position)
        gap_to_target = max(0, target_position - position)
        best_bid = int(book["best_bid"])
        best_ask = int(book["best_ask"])

        if buy_capacity > 0 and gap_to_target >= self.PEPPER_PASSIVE_TRIGGER_GAP:
            bid_quote = min(
                best_bid + self.PEPPER_PASSIVE_BID_IMPROVE,
                int(math.floor(forward_fair - self.PEPPER_PASSIVE_MAKER_EDGE)),
            )
            if bid_quote < best_ask:
                bid_size = min(buy_capacity, gap_to_target, self.PEPPER_PASSIVE_MAX_SIZE)
                if bid_size > 0:
                    orders.append(Order(product, int(bid_quote), int(bid_size)))

        # 4) Small passive offer to recycle rich inventory without giving up carry.
        sell_capacity = max(0, limit + position)
        if position >= max(target_position, 68) + 4:
            ask_quote = max(best_ask - 1, int(math.ceil(base_fair + 6.0 + 0.25 * alpha)))
            if ask_quote > best_bid:
                ask_size = min(position - max(target_position, 68), 4)
                if ask_size > 0:
                    orders.append(Order(product, int(ask_quote), int(-ask_size)))

        return self.enforce_pepper_limits(limit, original_position, orders)
    ...
    def ensure_within_limits(self, product: str, position: int, orders: List[Order]) -> List[Order]:
        ...
        buy_orders = [(idx, order) for idx, order in indexed_orders if int(order.quantity) > 0]
        sell_orders = [(idx, order) for idx, order in indexed_orders if int(order.quantity) < 0]

        kept: List[tuple[int, Order]] = []

        for idx, order in sorted(buy_orders, key=lambda item: (int(item[1].price), item[0])):
            allowed = min(int(order.quantity), buy_remaining)
            if allowed > 0:
                kept.append((idx, Order(order.symbol, order.price, allowed)))
                buy_remaining -= allowed

        for idx, order in sorted(sell_orders, key=lambda item: (-int(item[1].price), item[0])):
            allowed = min(-int(order.quantity), sell_remaining)
            if allowed > 0:
                kept.append((idx, Order(order.symbol, order.price, -allowed)))
    ...
    def enforce_pepper_limits(self, limit: int, position: int, orders: List[Order]) -> List[Order]:
        running = position
        filtered: List[Order] = []
        for order in orders:
