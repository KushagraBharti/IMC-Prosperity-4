...
class Trader:
    LIMIT = 10
    CONFIG = {
        "MICROCHIP_SQUARE": ("reversal", 25, 0.90),
        "MICROCHIP_RECTANGLE": ("reversal", 25, 1.05),
        "ROBOT_DISHES": ("reversal", 1, 0.95),
        "ROBOT_MOPPING": ("reversal", 50, 1.20),
        "UV_VISOR_AMBER": ("momentum", 50, 1.00),
        "OXYGEN_SHAKE_GARLIC": ("momentum", 50, 1.05),
        "PANEL_1X4": ("momentum", 50, 1.05),
        "TRANSLATOR_VOID_BLUE": ("reversal", 50, 1.10),
        "GALAXY_SOUNDS_PLANETARY_RINGS": ("reversal", 50, 1.10),
    }
    ...
    def run(self, state: TradingState):
        ...
        for product, (mode, lookback, threshold) in self.CONFIG.items():
            ...
            if len(hist) <= lookback + 2:
                continue
            signal = (hist[-1] - hist[-1 - lookback]) / max(self.vol(hist[-90:]), 1.0)
            if mode == "reversal":
                signal = -signal
        ...
        for _, product, book, signal, threshold in scored[:8]:
    ...
    def trade(self, product: str, book: dict, pos: int, signal: float, threshold: float) -> List[Order]:
        if signal > 0 and pos < self.LIMIT:
            ...
            return [Order(product, price, self.LIMIT - pos if signal > threshold + 1.0 else min(6, self.LIMIT - pos))]
        if signal < 0 and pos > -self.LIMIT:
