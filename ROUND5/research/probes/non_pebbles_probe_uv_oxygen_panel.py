...
class Trader:
    LIMIT = 10
    CONFIG = {
        "UV_VISOR_AMBER": ("momentum", 50, 0.95),
        "UV_VISOR_YELLOW": ("reversal", 50, 1.05),
        "OXYGEN_SHAKE_GARLIC": ("momentum", 50, 1.00),
        "PANEL_1X4": ("momentum", 50, 1.00),
        "PANEL_2X4": ("momentum", 25, 1.05),
    }
    ...
    def trade(self, product: str, book: dict, pos: int, signal: float, threshold: float) -> List[Order]:
        ...
        if signal > 0 and pos < self.LIMIT:
            ...
            return [Order(product, price, self.LIMIT - pos if signal > threshold + 0.8 else min(6, self.LIMIT - pos))]
        if signal < 0 and pos > -self.LIMIT:
