...
BACKTEST_DIR = OUTPUT_DIR / "backtests" / "candidate_6_10"
STRATEGIES = [f"round5_candidate_{i}.py" for i in range(6, 11)]
...
def write_tables(rows: list[dict[str, Any]]) -> None:
    ...
    with (OUTPUT_DIR / "candidate_6_10_score_table.csv").open("w", encoding="utf-8", newline="") as handle:
    ...
    lines.append("")
    lines.append("Rust Full skipped: not fast enough to justify routine batch scoring; Kevin/Xeeshan plus official-window replay are the active comparison tools.")
    (OUTPUT_DIR / "candidate_6_10_score_table.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    config = read_json(ROOT / "config" / "tools.local.json")
    full_roots = {
        "kevin": ROOT / "outputs" / "tool-data" / "kevin",
        "xeeshan": ROOT / "outputs" / "tool-data" / "xeeshan",
    }
    portal_root = OUTPUT_DIR / "official_portal_windows" / "round5_candidate_1"
    if not portal_root.exists():
        raise SystemExit(f"Missing official portal window root: {portal_root}")

    raw: dict[str, dict[str, Any]] = {}
    rows: list[dict[str, Any]] = []
    for strategy in STRATEGIES:
        print(f"Scoring {strategy}...")
        raw[strategy] = {
            "kevin_full": run_tool("kevin", strategy, "5", full_roots["kevin"], "full", config),
            "xeeshan_full": run_tool("xeeshan", strategy, "5", full_roots["xeeshan"], "full", config),
    ...
    (OUTPUT_DIR / "candidate_6_10_raw_backtests.json").write_text(json.dumps(raw, indent=2), encoding="utf-8")
    write_tables(rows)
