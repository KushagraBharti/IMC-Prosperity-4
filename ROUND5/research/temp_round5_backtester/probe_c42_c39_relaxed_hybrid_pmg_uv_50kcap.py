...
# Temporary fresh-edge pivot probe: probe_pivot_robust_panel
# Hand-integrated mechanism probe: probe_hand_robust_panel_micro_anchor
class Trader:
    LIMIT = 10
    ANCHOR = 10000
    ...
    PEB_AGGRESSION = 1.08
    PEB_ALLOW_TAKE = 1
    SIGNAL_CONFIG = {'OXYGEN_SHAKE_GARLIC': ('reversal', 200, 0.78, 1.35, 'passive'),
     ...
     'GALAXY_SOUNDS_DARK_MATTER': ('breakout_high', 200, 0.9, 0.95, 'passive'),
     'GALAXY_SOUNDS_SOLAR_FLAMES': ('rolling_mean_reversion', 200, 0.9, 0.9, 'passive'),
     'GALAXY_SOUNDS_SOLAR_WINDS': ('rolling_mean_reversion', 200, 0.96, 0.8, 'passive'),
     'GALAXY_SOUNDS_PLANETARY_RINGS': ('breakout_low_reversal', 200, 0.78, 1.15, 'passive'),
     'TRANSLATOR_SPACE_GRAY': ('breakout_high', 500, 0.86, 1.2, 'passive'),
     ...
     'OXYGEN_SHAKE_CHOCOLATE': ('rolling_mean_reversion', 200, 0.9, 0.95, 'passive'),
     ...
     'SLEEP_POD_COTTON': ('momentum', 100, 0.9, 1.1, 'passive'),
     ...
     'TRANSLATOR_GRAPHITE_MIST': ('momentum', 100, 1.02, 1.0, 'passive'),
     ...
     'SLEEP_POD_SUEDE': ('reversal', 100, 0.9, 1.1, 'passive'),
     'GALAXY_SOUNDS_BLACK_HOLES': ('reversal', 50, 0.92, 0.9, 'passive'),
     'TRANSLATOR_ASTRO_BLACK': ('momentum', 100, 0.94, 0.95, 'passive'),
     'SNACKPACK_STRAWBERRY': ('reversal', 200, 0.98, 0.8, 'passive'),
     'SNACKPACK_RASPBERRY': ('reversal', 200, 1.0, 0.75, 'passive'),
     'SNACKPACK_PISTACHIO': ('reversal', 200, 1.0, 0.65, 'passive'),
     'ROBOT_VACUUMING': ('reversal', 200, 1.02, 0.55, 'passive'),
     'MICROCHIP_TRIANGLE': ('reversal', 150, 0.82, 0.7, 'hybrid'),
     'UV_VISOR_AMBER': ('momentum', 150, 0.7, 0.76, 'hybrid'),
     'SNACKPACK_CHOCOLATE': ('momentum', 200, 0.98, 0.7, 'passive'),
     'SNACKPACK_VANILLA': ('momentum', 200, 1.02, 0.65, 'passive')}
    MAX_SIGNAL_PRODUCTS = 42
    MICRO_ANCHOR_EDGE = 16.0
    MICRO_ANCHOR_SIZE = 10
    MICRO_ANCHOR_TAKE_EXTRA = 2.0
    HYBRID_PRODUCTS = {'PANEL_4X4','PANEL_1X2','PANEL_1X4','PANEL_2X2','PANEL_2X4','MICROCHIP_OVAL','MICROCHIP_SQUARE','MICROCHIP_RECTANGLE','MICROCHIP_TRIANGLE','UV_VISOR_ORANGE','UV_VISOR_RED','UV_VISOR_YELLOW','UV_VISOR_AMBER','GALAXY_SOUNDS_PLANETARY_RINGS','GALAXY_SOUNDS_DARK_MATTER','GALAXY_SOUNDS_SOLAR_FLAMES','GALAXY_SOUNDS_SOLAR_WINDS','GALAXY_SOUNDS_BLACK_HOLES'}
    ...
    def run_signals(self, state: TradingState, cache: dict, result: Dict[str, List[Order]]) -> None:
        ...
        for product, cfg in self.SIGNAL_CONFIG.items():
            ...
            if product in self.HYBRID_PRODUCTS:
                style = "hybrid"
    ...
    def trade_signal(self, product: str, book: dict, pos: int, signal: float, threshold: float, weight: float, style: str) -> List[Order]:
        ...
        if signal > 0 and pos < self.LIMIT:
            price = book["ask"] if style == "hybrid" and abs(signal) > threshold + 0.0 else self.improve_bid(book)
            return [Order(product, price, min(base_qty, self.LIMIT - pos))]
        if signal < 0 and pos > -self.LIMIT:
            price = book["bid"] if style == "hybrid" and abs(signal) > threshold + 0.0 else self.improve_ask(book)
            return [Order(product, price, -min(base_qty, self.LIMIT + pos))]
        return []
