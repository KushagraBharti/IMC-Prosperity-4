...
class Trader:
    ...
    HYBRID_PRODUCTS = {'PANEL_4X4','PANEL_1X2','PANEL_1X4','PANEL_2X2','PANEL_2X4','GALAXY_SOUNDS_PLANETARY_RINGS','GALAXY_SOUNDS_DARK_MATTER','GALAXY_SOUNDS_SOLAR_FLAMES','GALAXY_SOUNDS_SOLAR_WINDS','GALAXY_SOUNDS_BLACK_HOLES'}
    ...
    def improve_bid(self, book: dict) -> int:
        return min(book["bid"] + (2 if book["spread"] >= 5 else 1), book["ask"] - 1) if book["spread"] >= 3 else book["bid"]

    def improve_ask(self, book: dict) -> int:
        return max(book["ask"] - (2 if book["spread"] >= 5 else 1), book["bid"] + 1) if book["spread"] >= 3 else book["ask"]
