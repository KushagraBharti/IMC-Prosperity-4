...
class Trader:
    ...
    PEB_BOOST = {'PEBBLES_XL': 1.15, 'PEBBLES_M': 1.18, 'PEBBLES_L': 1.08, 'PEBBLES_S': 0.92, 'PEBBLES_XS': 0.72}
    PEB_AGGRESSION = 1.08
    PEB_ALLOW_TAKE = 1
    ...
    def run_pebbles(self, state: TradingState, cache: dict, result: Dict[str, List[Order]]) -> None:
        ...
        for product in self.PEBBLES:
            ...
            if product == 'PEBBLES_XS':
                inv_fair = 2 * books[product]['mid'] - fairs[product]
                result[product] = self.quote_pebble(product, books[product], inv_fair, state.position.get(product, 0), rvol)
            else:
