...
# Temporary PEBBLES fair-value probe: probe_peb_portal_quadratic_taker_mxl
class Trader:
    LIMIT = 10
    ANCHOR = 10000
    ...
    PEB_BOOST = {'PEBBLES_XL': 1.15, 'PEBBLES_M': 1.28, 'PEBBLES_L': 1.22, 'PEBBLES_S': 0.92, 'PEBBLES_XS': 1.16}
    PEB_AGGRESSION = 1.24
    PEB_ALLOW_TAKE = 1
    ...
    def quote_pebble(self, product: str, book: dict, fair: float, pos: int, rvol: float) -> List[Order]:
        ...
        if self.PEB_ALLOW_TAKE and abs(book["mid"] - fair) > max(7.5, 1.35 * rvol):
            if fair - book["ask"] > edge_floor + 3.8 and pos < self.LIMIT:
