...
class Trader:
    ...
    HYBRID_PRODUCTS = {'PANEL_4X4','PANEL_1X2','PANEL_1X4','PANEL_2X2','PANEL_2X4','GALAXY_SOUNDS_PLANETARY_RINGS','GALAXY_SOUNDS_DARK_MATTER','GALAXY_SOUNDS_SOLAR_FLAMES','GALAXY_SOUNDS_SOLAR_WINDS','GALAXY_SOUNDS_BLACK_HOLES','TRANSLATOR_ECLIPSE_CHARCOAL','TRANSLATOR_SPACE_GRAY','TRANSLATOR_GRAPHITE_MIST','TRANSLATOR_ASTRO_BLACK','TRANSLATOR_VOID_BLUE'}
    ...
    def run_signals(self, state: TradingState, cache: dict, result: Dict[str, List[Order]]) -> None:
        ...
        for product, cfg in self.SIGNAL_CONFIG.items():
            ...
            elif pos > 0 and signal < threshold * 0.25:
                result[product] = [Order(product, self.improve_ask(book), -min(4, pos))]
            elif pos < 0 and signal > -threshold * 0.25:
                result[product] = [Order(product, self.improve_bid(book), min(4, -pos))]
