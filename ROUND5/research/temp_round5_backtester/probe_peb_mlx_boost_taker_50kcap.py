...
# Temporary executable 150k probe: probe_peb_mlx_boost_taker
class Trader:
    LIMIT = 10
    PEBBLES = ['PEBBLES_XS', 'PEBBLES_S', 'PEBBLES_M', 'PEBBLES_L', 'PEBBLES_XL']
    PEB_X = {"PEBBLES_XS": 1.0, "PEBBLES_S": 2.0, "PEBBLES_M": 3.0, "PEBBLES_L": 4.0, "PEBBLES_XL": 5.0}
    PEB_BOOST = {'PEBBLES_XL': 1.15, 'PEBBLES_M': 1.35, 'PEBBLES_L': 1.25, 'PEBBLES_S': 0.92, 'PEBBLES_XS': 1.15}
    PEB_AGGRESSION = 1.22
    PEB_ALLOW_TAKE = 1
    ...
    def quote_pebble(self, product: str, book: dict, fair: float, pos: int, rvol: float) -> List[Order]:
        ...
        if self.PEB_ALLOW_TAKE and abs(book["mid"] - fair) > max(8.0, 1.45 * rvol):
            if fair - book["ask"] > edge_floor + 4.2 and pos < self.LIMIT:
