...
class Trader:
    ...
    def trade_anchor(self, product: str, depth, position: int) -> List[Order]:
        ...
        if bid + 1 < ask:
            if position < self.LIMIT:
                orders.append(Order(product, int(min(bid + 1, self.ANCHOR - 2)), int(min(10, self.LIMIT - position))))
            if position > -self.LIMIT:
                orders.append(Order(product, int(max(ask - 1, self.ANCHOR + 2)), int(-min(10, self.LIMIT + position))))
