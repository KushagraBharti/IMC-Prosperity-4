...
# Temporary missing-mechanism executable probe: probe_missing_portal_micro_anchor_plus_fill
class Trader:
    LIMIT = 10
    ANCHOR = 10000
    ...
    PRODUCT_ANCHOR_CONFIG = {'MICROCHIP_SQUARE': (14250.0, 8.0, 6, 'sell_high', 'passive')}
    PRODUCT_ANCHOR_OVERRIDE = True
    ONE_SIDED_FILL_CONFIG = {'OXYGEN_SHAKE_GARLIC': ('bid', 3, 0.05, 4, 4.0), 'UV_VISOR_YELLOW': ('ask', 3, 0.05, 4, 4.0), 'MICROCHIP_SQUARE': ('ask', 3, 0.05, 5, 4.0)}
    ONE_SIDED_FILL_OVERRIDE = True
    MAX_ONE_SIDED_FILL_PRODUCTS = 8
    def run(self, state: TradingState):
        cache = self.load_cache(state.traderData)
        result: Dict[str, List[Order]] = {}
        self.run_pebbles(state, cache, result)
        self.run_signals(state, cache, result)
        self.run_product_anchor_edges(state, result)
        self.run_one_sided_fill_edges(state, cache, result)
