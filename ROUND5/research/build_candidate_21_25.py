...
class Trader:
    ...
    SIGNAL_CONFIG = __SIGNAL_CONFIG__
    MAX_SIGNAL_PRODUCTS = __MAX_SIGNAL_PRODUCTS__
...
LONG = {
    "MICROCHIP_SQUARE": ("reversal", 100, 0.90, 1.10),
    "GALAXY_SOUNDS_PLANETARY_RINGS": ("momentum", 100, 0.95, 1.00),
    "ROBOT_LAUNDRY": ("momentum", 100, 0.95, 1.00),
    "OXYGEN_SHAKE_EVENING_BREATH": ("reversal", 100, 0.95, 1.00),
    "MICROCHIP_TRIANGLE": ("reversal", 50, 0.95, 1.00),
    "ROBOT_IRONING": ("momentum", 100, 0.95, 1.00),
    "UV_VISOR_AMBER": ("momentum", 100, 0.95, 1.00),
    "MICROCHIP_OVAL": ("momentum", 100, 1.05, 0.85),
    "SLEEP_POD_SUEDE": ("reversal", 100, 1.05, 0.85),
}

UNRESOLVED_CLEAN = {
    "UV_VISOR_ORANGE": ("momentum", 100, 0.95, 1.20),
    "SLEEP_POD_COTTON": ("momentum", 100, 0.95, 1.10),
    "TRANSLATOR_GRAPHITE_MIST": ("momentum", 100, 1.10, 0.90),
    "SLEEP_POD_POLYESTER": ("reversal", 100, 1.00, 0.90),
    "PANEL_2X4": ("momentum", 50, 1.00, 0.85),
    "ROBOT_MOPPING": ("momentum", 50, 1.00, 0.85),
    "TRANSLATOR_VOID_BLUE": ("reversal", 100, 1.00, 0.80),
    "MICROCHIP_RECTANGLE": ("momentum", 50, 1.00, 0.70),
    "SLEEP_POD_NYLON": ("reversal", 100, 1.05, 0.65),
    "SLEEP_POD_LAMB_WOOL": ("momentum", 100, 1.05, 0.55),
}

BEST_ADDITIVE = {
    "MICROCHIP_SQUARE": ("reversal", 100, 0.95, 1.00),
    "ROBOT_LAUNDRY": ("momentum", 100, 1.00, 0.95),
    "OXYGEN_SHAKE_EVENING_BREATH": ("reversal", 100, 1.00, 0.90),
    "MICROCHIP_TRIANGLE": ("reversal", 50, 1.00, 0.90),
    "ROBOT_IRONING": ("momentum", 100, 1.00, 0.90),
    "UV_VISOR_AMBER": ("momentum", 100, 1.00, 0.90),
    "UV_VISOR_ORANGE": ("momentum", 100, 1.00, 1.10),
    "SLEEP_POD_COTTON": ("momentum", 100, 1.00, 1.00),
    "TRANSLATOR_GRAPHITE_MIST": ("momentum", 100, 1.15, 0.70),
}


def write_candidate(num: int, pebbles: bool, config: dict[str, tuple[str, int, float, float]], max_products: int) -> None:
    text = HEADER + BASE
    text = text.replace("__PEBBLES__", repr(PEBBLES if pebbles else []))
    text = text.replace("__SIGNAL_CONFIG__", repr(config))
    text = text.replace("__MAX_SIGNAL_PRODUCTS__", str(max_products))
    (STRAT / f"round5_candidate_{num}.py").write_text(text, encoding="utf-8")


def main() -> None:
    write_candidate(21, False, LONG, 8)
    write_candidate(22, False, UNRESOLVED_CLEAN, 8)
    write_candidate(23, True, LONG, 8)
    write_candidate(24, True, UNRESOLVED_CLEAN, 7)
    write_candidate(25, True, BEST_ADDITIVE, 7)
