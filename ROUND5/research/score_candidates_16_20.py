...
BACKTEST_DIR = OUTPUT_DIR / "backtests" / "candidate_16_20"
STRATEGIES = [f"round5_candidate_{i}.py" for i in range(16, 21)]
...
def write_tables(rows: list[dict[str, Any]]) -> None:
    ...
    with (OUTPUT_DIR / "candidate_16_20_score_table.csv").open("w", encoding="utf-8", newline="") as handle:
    ...
    lines.append("")
    lines.append("Rust Full skipped by request.")
    (OUTPUT_DIR / "candidate_16_20_score_table.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    (OUTPUT_DIR / "candidate_16_20_raw_backtests.json").write_text(json.dumps(raw, indent=2), encoding="utf-8")
    write_tables(rows)
