...
BACKTEST_DIR = OUTPUT_DIR / "backtests" / "candidate_11_15"
STRATEGIES = [f"round5_candidate_{i}.py" for i in range(11, 16)]
...
def write_tables(rows: list[dict[str, Any]]) -> None:
    ...
    with (OUTPUT_DIR / "candidate_11_15_score_table.csv").open("w", encoding="utf-8", newline="") as handle:
    ...
    lines.append("")
    lines.append("Rust Full skipped: Kevin/Xeeshan plus official-window replay are the active tools; Rust was not fast enough to justify this batch run.")
    (OUTPUT_DIR / "candidate_11_15_score_table.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    (OUTPUT_DIR / "candidate_11_15_raw_backtests.json").write_text(json.dumps(raw, indent=2), encoding="utf-8")
    write_tables(rows)
