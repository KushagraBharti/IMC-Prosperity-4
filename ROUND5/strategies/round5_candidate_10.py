...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if book:
            ...
            if len(hist) >= 60:
                vol = self.vol(hist[-140:])
                long_z = self.zscore(hist, 500, vol)
                fast_z = self.zscore(hist, 120, vol)
                past = (hist[-1] - hist[max(0, len(hist) - 81)]) / max(vol, 1.0)
                signal = -0.78 * long_z - 0.32 * fast_z - 0.08 * past + 0.20 * book["imb"]
    ...
    def trade(self, book: dict, pos: int, signal: float) -> List[Order]:
        ...
        if abs(signal) < threshold:
            if pos > 5:
                return [Order(self.PRODUCT, book["bid"], -min(6, pos))]
            if pos < -5:
                return [Order(self.PRODUCT, book["ask"], min(6, -pos))]
            return []
        size = 5 if abs(signal) < threshold + 0.55 else 10
        take = abs(signal) > threshold + 0.30 and book["spread"] <= 18
