...
class Trader:
    ...
    EXTRA_PRODUCTS = {
        ...
        "PANEL_1X2": (1.55, 0.70),
        "SLEEP_POD_SUEDE": (0.85, 0.45),
        "SLEEP_POD_NYLON": (0.80, 0.50),
    }
    MOMENTUM_EXTRAS = {
        ...
        "GALAXY_SOUNDS_BLACK_HOLES": (220, 2.00, -0.30),
    ...
    def trade_momentum_extra(self, product: str, book: dict, position: int, signal: float, threshold: float) -> List[Order]:
        intensity = min(1.0, max(0.0, (abs(signal) - threshold) / 0.9))
        target = int(round((self.LIMIT if signal > 0 else -self.LIMIT) * intensity))
        delta = target - position
        if delta > 0:
            price = book["ask"] if abs(signal) > threshold + 1.1 else self.improve_bid(book)
            return self.ensure_limit(product, position, [Order(product, int(price), int(delta))])
        if delta < 0:
            price = book["bid"] if abs(signal) > threshold + 1.1 else self.improve_ask(book)
            return self.ensure_limit(product, position, [Order(product, int(price), int(delta))])
        return []
