...
class Trader:
    ...
    VELVET_REL_TAKE_EDGE = 6.0
    VELVET_REL_SKEW = 0.03
    VELVET_REL_TAKE_SIZE = 30
    VELVET_REL_PASSIVE_SIZE = 8
    ...
    def run(self, state: TradingState):
        ...
        if hydro_book and hydro_book["has_both_sides"]:
            ...
            if timestamp >= self.TRADE_START_TIMESTAMP:
                results["HYDROGEL_PACK"] = self.trade_signal_product(
        ...
        if velvet_book and velvet_book["has_both_sides"]:
            velvet_fair = self.compute_linear_fair(
                cache=cache,
                key="velvet_anchor",
            ...
            velvet_fair = 0.35 * velvet_fair + 0.65 * self.vfe_implied_fair(books)
        ...
        if timestamp >= self.TRADE_START_TIMESTAMP:
            option_orders = self.trade_vouchers_relative(books, state.position, velvet_fair, t_years)
    ...
    def trade_vouchers_relative(
        ...
        for symbol in self.ACTIVE_VOUCHERS:
            ...
            fair_adj = fair - position * self.OPTION_INV_SKEW - 0.0015 * net_delta * delta
            original_position = position
            orders: List[Order] = []
    ...
    def time_to_expiry_years(self, timestamp: int) -> float:
        days_remaining = max(0.25, 5.0 - (timestamp / 1_000_000.0))
