...
class Trader:
    ...
    def trade_osmium(
        ...
        # Layer a second passive order at the current top of book. On the official
        # engine this helps us harvest more neutral flow without changing the core
        # fair-value logic.
        buy_capacity = max(0, limit - position)
        sell_capacity = max(0, limit + position)
        ...
        if buy_capacity > 0 and best_bid < best_ask:
            orders.append(
                Order(
                    product,
                    int(best_bid),
                    int(min(self.OSMIUM_SECONDARY_SIZE, buy_capacity)),
                )
            )
        ...
        if sell_capacity > 0 and best_ask > best_bid:
            orders.append(
                Order(
                    product,
                    int(best_ask),
                    int(-min(self.OSMIUM_SECONDARY_SIZE, sell_capacity)),
                )
            )
    ...
    def ensure_within_limits(self, product: str, position: int, orders: List[Order]) -> List[Order]:
        ...
        # If we must trim same-side exposure, preserve the highest-edge prices first:
        # lowest prices for buys, highest prices for sells.
