...
class Trader:
    ...
    # Strongest default submission
    EMERALDS_FAIR = 10000.0
    EMERALDS_BASE_SIZE = 14
    EMERALDS_TAKE_EDGE = 1.0
    EMERALDS_MAKER_EDGE = 1.0
    EMERALDS_QUOTE_IMPROVE = 2
    EMERALDS_SKEW = 0.10
    EMERALDS_SOFT_LIMIT = 42
    EMERALDS_FLATTEN_TRIGGER = 24
    EMERALDS_ENDGAME = 180000

    TOMATOES_BASE_SIZE = 13
    TOMATOES_TAKE_EDGE = 1.0
    TOMATOES_MAKER_EDGE = 1.0
    TOMATOES_QUOTE_IMPROVE = 2
    TOMATOES_SKEW = 0.16
    TOMATOES_SOFT_LIMIT = 36
    TOMATOES_FLATTEN_TRIGGER = 22
    TOMATOES_ENDGAME = 168000
    TOMATOES_DEVIATION_WEIGHT = 1.10
    TOMATOES_IMBALANCE_WEIGHT = 2.10
    TOMATOES_TREND_WEIGHT = 0.45
    TOMATOES_REVERSAL_GATE = 0.60
    ...
    def trade_tomatoes(self, depth: OrderDepth, position: int, timestamp: int, cache: Dict[str, List[float]]) -> List[Order]:
        ...
        imbalance_trend = self.trend_signal(cache.get("tomato_imbalance", []))

        alpha = (
            deviation * self.TOMATOES_DEVIATION_WEIGHT
            + imbalance * self.TOMATOES_IMBALANCE_WEIGHT
            + trend * self.TOMATOES_TREND_WEIGHT
            + imbalance_trend * 0.35
        )
        alpha = self.clip(alpha, -3.5, 3.5)
        reservation = wall + alpha - position * self.TOMATOES_SKEW
        sweep_levels = 3 if abs(alpha) >= 1.6 or abs(deviation) >= 1.0 else 2
    ...
    def generate_orders(
        ...
        bid_scale = self.clip(1.0 - 1.05 * long_pressure + 0.30 * short_pressure, 0.05, 1.40)
        ask_scale = self.clip(1.0 - 1.05 * short_pressure + 0.30 * long_pressure, 0.05, 1.40)
        ...
        for level, (ask_price, ask_volume) in enumerate(sell_orders):
            ...
            edge = fair - ask_price
            flatten_buy = position < -flatten_trigger and edge >= -0.25
            signal_ok = signal >= -self.TOMATOES_REVERSAL_GATE or product == "EMERALDS"
            if edge >= take_edge or (allow_zero_edge_flatten and flatten_buy):
                if signal_ok or flatten_buy or edge >= 2.0:
                    size = min(
                        buy_capacity,
                        available,
        ...
        for level, (bid_price, bid_volume) in enumerate(buy_orders):
            ...
            edge = bid_price - fair
            flatten_sell = position > flatten_trigger and edge >= -0.25
            signal_ok = signal <= self.TOMATOES_REVERSAL_GATE or product == "EMERALDS"
            if edge >= take_edge or (allow_zero_edge_flatten and flatten_sell):
                if signal_ok or flatten_sell or edge >= 2.0:
                    size = min(
                        sell_capacity,
                        available,
