...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Replay round for the replay backtesters. Examples: tutorial, round1, round2, round3, round4, round5.",
    )
    parser.add_argument(
        "--days",
        nargs="*",
...
def normalize_round_key(round_key: str) -> str:
    ...
    if normalized in {"3", "round3", "round_3"}:
        return "round3"
    if normalized in {"4", "round4", "round_4"}:
        return "round4"
    if normalized in {"5", "round5", "round_5"}:
        return "round5"
    return normalized
